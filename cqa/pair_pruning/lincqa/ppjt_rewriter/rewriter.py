...
def get_var_indices_in_atom(var, atom):
    ret = []
    for i in range(len(atom.arguments)):
        atom_var = atom.arguments[i]
        if var.name.upper() == atom_var.name.upper():
            ret.append(i)
    return ret


def get_candidate_projection_attributes(ppjt, schema):

    all_atoms = ppjt.get_all_atoms()





def ppjt_sql_rewrite(ppjt, schema):


    get_candidate_projection_attributes(ppjt, schema)
